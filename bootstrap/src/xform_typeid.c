/**
 * @file xform_typeid.c
 * @brief Transforms `typeid(type)` into an encoded type identifier.
 */

#include <ctype.h>
#include "base_alloc_shim.h"

#include "branch_symbols.h"
#include "transform.h"
#include "rewrite.h"
#include "types.h"
#include "token.h"
#include "nt_types.h"
#include "assert.h"

// Transform:
// typeid(type_name|string [, typename|string_literal]*) -> encoded identifier

static void
extract_atom(xform_ctx_t *ctx, tnode_t *node, list_t **l)
{
    char *text;

    if (!node) {
        return;
    }

    assert(node->nt_id == NT_typeid_atom);
    node = tnode_get_kid(node, 0);
    if (node->nt_id == NT_typeof_specifier_argument) {
        text = normalize_type_node(node, ctx->input);
    }
    else {
        // string_literal node — may span multiple adjacent tokens
        // (e.g., "prefix_" "foo" "_" from macro stringification).
        int num_toks = node->num_toks;
        // Drill down to the terminal with the first token.
        while (node && !node->tptr) {
            node = tnode_get_kid(node, 0);
        }
        assert(node && node->tptr);
        if (num_toks < 1) {
            num_toks = 1;
        }

        // Extract and concatenate all adjacent string tokens, stripping quotes.
        // Tokens may not be contiguous in the array (whitespace tokens between).
        list_t *parts   = list_alloc(0);
        tok_t  *cur_tok = node->tptr;
        int     found   = 0;
        while (found < num_toks) {
            if (cur_tok->type == TT_STR) {
                char *part = extract(ctx->input, cur_tok);
                int   slen = strlen(part);
                if (slen >= 2 && part[0] == '"' && part[slen - 1] == '"') {
                    memmove(part, part + 1, slen - 2);
                    part[slen - 2] = '\0';
                }
                parts = list_append(parts, part);
                found++;
            }
            cur_tok++;
        }
        text = join(parts, "");
        base_dealloc(parts);

        // Validate that the result is a valid C identifier fragment.
        for (char *p = text; *p; p++) {
            if (!isalnum((unsigned char)*p) && *p != '_') {
                ncc_error(
                    "%s:%d: typeid() string literal contains "
                    "invalid identifier character '%c'\n",
                    ctx->lex->in_file,
                    get_node_line(node),
                    *p);
                exit(1);
            }
        }
    }
    *l = list_append(*l, text);
}

static tnode_t *
xform_typeid(xform_ctx_t *ctx, tnode_t *node)
{
    // We're going to get a synthetic typeid node.
    if (node->branch != BRANCH(synthetic_identifier, TYPEID)) {
        return nullptr;
    }
    // kids: [0]=typeid [1]=( [2]=typeid_atom [3]=opt(continuation) [4]=)
    list_t *flattened = list_alloc(0);

    extract_atom(ctx, tnode_get_kid(node, 2), &flattened);

    tnode_t *next = tnode_get_kid(node, 3);

    while (next && next->nt_id == NT_typeid_continuation) {
        // Extract the atom, which only exists on branch 0, at index
        // 1.  If it's branch 1 (trailing comma), will pass nullptr to
        // extract_atom(), which is fine.
        extract_atom(ctx, tnode_get_kid(next, 1), &flattened);

        // Same thing here. Next continuation is in slot 2, or is nullptr.
        next = tnode_get_kid(next, 2);
    }

    char    *type_id = join(flattened, "");
    tnode_t *id_node = synth_terminal(type_id, TT_ID, get_node_line(node));
    return replace_node(node, id_node, "typeid");
}

/**
 * @brief Register the typeid transformation.
 *
 * @param reg Registry to add the transformation to
 */
void
register_typeid_xform(xform_registry_t *reg)
{
    xform_register_post(reg, NT_synthetic_identifier, xform_typeid, "typeid");
}
