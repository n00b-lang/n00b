...
- [x] (2026-03-19 01:23Z) Applied the follow-up review remediation from `plans/wave1-grid-review-remediation-execplan.md`: added regression coverage for row-gap spans, plain planes, oversized track counts, stale spans, and post-layout manual moves; fixed the shared runtime geometry path; and corrected the public `n00b_grid_set_tracks()` narrative.
...
- Observation: The original grid landing also changed shared runtime assumptions indirectly by relying on absolute layout bounds for child positioning. Evidence: `plans/wave1-grid-review-remediation-execplan.md` later had to add shared fixes in `src/display/render/composite.c`, `src/display/mouse.c`, and `src/display/render/plane.c` after new regression tests proved that manual `n00b_plane_move()` calls left rendering and hit-testing behind the moved plane.
...
- Decision: The initial production landing assumed grid would not require shared mouse or compositor changes, but that assumption was later disproved and superseded by `plans/wave1-grid-review-remediation-execplan.md`. Rationale: once grid children stored absolute layout bounds, manual `n00b_plane_move()` calls exposed a shared runtime inconsistency between `plane->x/y` and `plane->bounds.x/y`; the stable fix lived in the shared geometry path, not inside the widget alone. Date/Author: 2026-03-19 / Codex.
...
- Decision: The shipped `n00b_grid_set_tracks(..., n00b_isize_t count)` contract is safe-clear on `tracks == nullptr`, `count == 0`, and oversized counts above `INT32_MAX`; negative sentinels are not part of the public API. Rationale: `n00b_isize_t` is unsigned in Athens, so the original `count <= 0` wording was not representable safely and later review remediation hardened the implementation to match the corrected documentation. Date/Author: 2026-03-19 / Codex.

## Outcomes & Retrospective

As of 2026-03-19 this plan remains executed successfully, but it is no longer the whole story of the grid landing. Athens still has the production `grid` container, public header, unit tests, demo entrypoint, and documentation described here, and those core acceptance commands continue to pass.

What changed after the original landing is captured in `plans/wave1-grid-review-remediation-execplan.md`. That follow-up added missing regression coverage, fixed shared-runtime manual-move behavior, corrected plain-plane and stale-span handling, and hardened `n00b_grid_set_tracks()` against oversized unsigned counts. The result is a more accurate final history: the first landing shipped the widget successfully, but review remediation was required before the full grid contract matched the implementation and shared runtime behavior.
...
Milestone 2 adds focused automated proof. Create `test/unit/test_grid.c` using the same style as `test_zstack.c`: a small dummy widget vtable, stream-canvas helpers, per-test `printf("  [PASS] ...")` output, and explicit assertions. The test file must prove the public API surface, fixed-column placement, span behavior, deterministic `FR` remainder handling, auto-fit column selection, hidden-child skipping, and normal click routing after layout. The original landing kept the matrix focused on the new `grid` test plus the existing shared mouse and event-dispatch tests, but the later review remediation expanded the permanent proof set to include `render_canvas`, extra `mouse` coverage, and new grid regressions for plain planes, stale spans, row-gap spans, and oversized track counts.
...
5. Create `src/display/widgets/grid.c`. Keep the shared public contract small and put all layout helpers in this file. The file should define `grid_destroy`, `grid_render`, `grid_measure`, `grid_handle_event`, `grid_can_focus`, and `grid_layout`, plus private helper functions that make measurement and layout share the same placement math.
   ...
   `n00b_grid_set_tracks()` copies the provided track array into widget-owned storage, sets `track_count = count`, sets `columns = count`, clears auto-fit mode, and marks the plane dirty. Passing `nullptr`, `count == 0`, or an oversized count above `INT32_MAX` clears track mode and falls back to the current fixed-column count.
...
If `test_grid` fails, fix the grid widget or the new test first before widening scope, but do not assume the problem is automatically widget-local. The later review remediation proved that grid's absolute-bounds layout can expose shared mouse and compositor bugs, so `test_mouse` and `test_render_canvas` are valid next stops when grid-specific logic looks correct. If the demo launches but keyboard interaction does not work, check `widget_demo.c` first and confirm the `"grid"` branch sets `use_event_loop = true` before assuming a focus-manager bug. Do not revert unrelated user changes in the worktree.

Change note (2026-03-19 / Codex): Updated after executing `plans/wave1-grid-review-remediation-execplan.md` so this earlier plan records the later shared-runtime fixes, the corrected `n00b_grid_set_tracks()` contract, and the fact that the original "widget-only" assumption was superseded in review remediation.
