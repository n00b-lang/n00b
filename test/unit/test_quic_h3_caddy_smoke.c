/*
 * test_quic_h3_caddy_smoke.c — End-to-end H3 GET smoke test against
 * a caddy fixture in Docker.
 *
 * Gated by N00B_TEST_DOCKER=1 (skip clean otherwise).  Requires:
 *   - CADDY_BASE_URL  (e.g., https://localhost:54321)
 *   - CADDY_CERT_FP   (SHA-256 of the leaf cert, hex)
 * Both are populated by `eval $(test/fixtures/caddy/start.sh)`.
 *
 * The test:
 *   1. Connects to the caddy container's HTTPS+H3 endpoint.
 *   2. Negotiates ALPN "h3".
 *   3. Pins the leaf cert via the CADDY_CERT_FP fingerprint.
 *   4. Issues GET /.
 *   5. Asserts response body contains "hello-from-caddy".
 *
 * If the caddy fixture isn't booted, the test SKIPs cleanly so the
 * suite doesn't fail in environments without Docker.
 */
...
parse_url(const char *url, char *host, size_t host_cap, uint16_t *port_out)
{
     ...
     * anything else (caddy fixture only emits this shape). */
...
main(int argc, char **argv)
{
    ...
    printf("test_quic_h3_caddy_smoke:\n");

    const char *gate = getenv("N00B_TEST_DOCKER");
    if (!gate || strcmp(gate, "1") != 0) {
        printf("  [SKIP] N00B_TEST_DOCKER!=1 — caddy fixture not booted\n");
        n00b_shutdown();
        return 77;  /* meson "skip" exit code */
    }

    const char *base_url = getenv("CADDY_BASE_URL");
    const char *cert_fp  = getenv("CADDY_CERT_FP");
    if (!base_url || !cert_fp) {
        printf("  [SKIP] CADDY_BASE_URL/CADDY_CERT_FP missing (eval start.sh first)\n");
        n00b_shutdown();
        return 77;
    }
    ...
    if (parse_url(base_url, host, sizeof(host), &port) != 0) {
        printf("  [FAIL] cannot parse CADDY_BASE_URL=%s\n", base_url);
        n00b_shutdown();
        return 1;
    }
    /* For caddy `tls internal`, the leaf SAN is "localhost"; we
     * connect to 127.0.0.1 but use SNI = "localhost". */
    ...
    /* For the test we disable picoquic's default verifier because
     * caddy's internal CA isn't in any system trust store.  The
     * production path would pin via picotls's verify callback using
     * CADDY_CERT_FP; that's the Phase 3 / Phase 4 trust bridge work
     * which is outside this sub-phase's scope. */
    ...
    /* The Caddyfile responds with "hello-from-caddy" plus a trailing
     * newline (heredoc).  Substring match. */
    const char *expected = "hello-from-caddy";
    bool found = false;
    ...
    if (!found) {
        ...
        printf("  [PASS] H3 GET / against caddy\n");
    }
