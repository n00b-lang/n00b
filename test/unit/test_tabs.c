...
test_tabs_focus_returns_to_header_when_hidden_page_was_focused(void)
{
    ...
    assert(n00b_focus_mgr_current_plane(fm) == focused_box);

    assert(n00b_tabs_select_index(tabs, 1));
    assert(n00b_focus_mgr_current_plane(fm) == tabs);
...
test_tabs_switch_clears_hidden_page_capture(void)
{
    ...
    assert(n00b_canvas_get_mouse_capture_plane(canvas) == scroll_page);
    assert(scroll_state->dragging_vertical_thumb);

    assert(n00b_tabs_select_index(tabs, 1));
    assert(n00b_canvas_get_mouse_capture_plane(canvas) == nullptr);
    assert(!scroll_state->dragging_vertical_thumb);
