/*
 * test_quic_phase5_vm_smoke.c — meson wrapper around the Phase 5
 * § 5.9 bare-VM fixture script.
 *
 ...
 *
 * Set N00B_TEST_VM=1 to actually run the fixture; otherwise the
...
main(int argc, char **argv)
{
    ...
    const char *script = getenv("N00B_PHASE5_VM_SCRIPT");
    if (!script) {
        printf("test_quic_phase5_vm_smoke:\n"
               "  [SKIP] N00B_PHASE5_VM_SCRIPT not set\n");
        n00b_shutdown();
        return 77;
    }
    ...
    printf("  [PASS] systemd PID-1 container + n00b unit + assert\n");
    n00b_shutdown();
    return 0;
}
