...
{
    printf("%s abi=%u text_style=%u rcell=%u event=%u vtable=%u get_font_metrics_off=%u poll_event_off=%u\n",
           label,
           (unsigned)layout->abi_version,
           (unsigned)layout->text_style_size,
           (unsigned)layout->rcell_size,
           (unsigned)layout->event_size,
           (unsigned)layout->renderer_vtable_size,
           (unsigned)layout->renderer_vtable_get_font_metrics_off,
           (unsigned)layout->renderer_vtable_poll_event_off);
}
