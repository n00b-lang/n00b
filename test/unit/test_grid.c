...
static void
test_grid_column_mutation_relayouts_existing_bounds(void)
{
    n00b_canvas_t *canvas = make_stream_canvas(10, 40);
    n00b_plane_t  *grid = n00b_grid_new(.canvas = canvas, .columns = 1);
    n00b_plane_t  *top = make_dummy_child(10, 4, 5, 2, false);
    n00b_plane_t  *next = make_dummy_child(10, 4, 5, 2, false);

    n00b_plane_add_child(grid, top, 0, 0);
    n00b_plane_add_child(grid, next, 0, 0);
    ...
    assert_rect_eq(dummy_state(top)->last_bounds, 0, 0, 40, 4);
    assert_rect_eq(dummy_state(next)->last_bounds, 0, 4, 40, 4);

    n00b_grid_set_columns(grid, 2);
    n00b_display_scene_rerender_dirty(canvas);

    assert_rect_eq(dummy_state(top)->last_bounds, 0, 0, 20, 4);
    assert_rect_eq(dummy_state(next)->last_bounds, 20, 0, 20, 4);

    n00b_canvas_remove_plane(canvas, grid);
    destroy_plane_tree(grid);
    n00b_canvas_destroy(canvas);
    printf("  [PASS] grid column mutation relayouts existing bounds\n");
}
...
test_grid_nested_card_children_render_and_interact(void)
{
    ...
    assert(n00b_focus_mgr_current_plane(fm) == left_button);
    ...
    assert(n00b_focus_mgr_current_plane(fm) == right_button);
...
main(int argc, char **argv)
{
    ...
    test_grid_column_mutation_relayouts_existing_bounds();
