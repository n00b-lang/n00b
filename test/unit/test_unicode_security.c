...
{
    ASSERT(n00b_unicode_is_confusable(STR("hello"), STR("hello")));
}

TEST(test_script_restriction_ascii)
{
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("hello"));
...
TEST(test_script_restriction_single)
{
    ...
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("\xE3\x81\x82\xE3\x81\x84")); // あい
...
TEST(test_inherited_ignored)
{
    ...
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("a\xCC\x81"));
...
TEST(test_mixed_latin_cyrillic)
{
    ...
    // "p" + U+0430 + "ypal"
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("p\xD0\xB0ypal"));
...
TEST(test_japanese_mixed)
{
    ...
    n00b_unicode_restriction_level_t level = n00b_unicode_script_restriction(
        STR("\xE4\xB8\x96\xE3\x81\x82\xE3\x82\xA2"));
...
TEST(test_korean_mixed)
{
    // Hangul (한) + Han (字)
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("\xED\x95\x9C\xE5\xAD\x97"));
    ASSERT_EQ(level, N00B_UNICODE_RESTRICTION_HIGHLY_RESTRICTIVE);
}
...
TEST(test_latin_plus_one)
{
    ...
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("a\xCE\xB1"));
    ASSERT_EQ(level, N00B_UNICODE_RESTRICTION_MODERATELY_RESTRICTIVE);
}
...
TEST(test_has_mixed_scripts)
{
    ...
    ASSERT(n00b_unicode_has_mixed_scripts(STR("p\xD0\xB0ypal")));

    // Pure Latin → not mixed
    ASSERT(!n00b_unicode_has_mixed_scripts(STR("hello")));
}
...
TEST(test_script_extensions_middle_dot)
{
    ...
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("l\xC2\xB7l"));
    ASSERT_EQ(level, N00B_UNICODE_RESTRICTION_SINGLE_SCRIPT);
}
...
TEST(test_common_digits)
{
    ...
    n00b_unicode_restriction_level_t level
        = n00b_unicode_script_restriction(STR("abc123"));
