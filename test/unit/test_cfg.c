...
test_cfg_successor_predecessor(void)
{
    ...
    n00b_cfg_edge_t succs[8];
    int32_t ns = n00b_cfg_successors(r.cfg, r.cfg->entry_id, succs, 8);
    assert(ns > 0);

    // Exit block should have predecessors.
    n00b_cfg_edge_t preds[8];
    int32_t np = n00b_cfg_predecessors(r.cfg, r.cfg->exit_id, preds, 8);
    assert(np > 0);
