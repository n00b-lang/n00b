...
static void
test_parse_raw_ctrl_y(void)
{
    const unsigned char bytes[] = { 0x19 };
    byte_stream_t stream = {
        .data = bytes,
        .len  = sizeof(bytes),
    };
    ...
    printf("  [PASS] ansi parser raw ctrl-y\n");
}
...
static void
assert_malformed_sgr_rejected(const unsigned char *bytes, size_t len)
{
    byte_stream_t stream = {
        .data = bytes,
        .len  = len,
...
static void
test_reject_malformed_sgr_mouse(void)
{
    const unsigned char missing_x_y[] = {
        0x1B, '[', '<', '0', ';', 'M',
    };
    const unsigned char missing_y[] = {
        0x1B, '[', '<', '0', ';', '1', '0', 'M',
    };

    assert_malformed_sgr_rejected(missing_x_y, sizeof(missing_x_y));
    assert_malformed_sgr_rejected(missing_y, sizeof(missing_y));

    printf("  [PASS] ansi parser rejects malformed sgr mouse\n");
}
...
main(int argc, char **argv)
{
    ...
    test_parse_raw_ctrl_y();
    test_parse_sgr_mouse();
    test_reject_malformed_sgr_mouse();
