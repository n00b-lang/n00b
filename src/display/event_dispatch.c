                             ...
                             const n00b_event_t *event)
{
    ...
    if (event->type == N00B_EVENT_KEY) {
        ...
        if (key == N00B_KEY_TAB && !(mods & N00B_MOD_SHIFT)) {
            if (fm) {
                n00b_plane_t *before = n00b_focus_mgr_current_plane(fm);
                n00b_focus_mgr_next(fm);
                n00b_plane_t *after = n00b_focus_mgr_current_plane(fm);
        ...
        if (fm) {
            n00b_plane_t *focused = n00b_focus_mgr_current_plane(fm);
            if (focused) {
    ...
    if (event->type == N00B_EVENT_MOUSE) {
        ...
        result.handled       = true;
        result.focus_changed = fm && (before != n00b_focus_mgr_current_plane(fm));
    }
