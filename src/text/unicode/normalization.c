...
#include "internal/text/unicode/hangul.h"
...
static int
get_canon_decomp(n00b_codepoint_t cp, n00b_codepoint_t *out)
{
    ...
    const uint32_t *entry = n00b_unicode_sparse_lookup(n00b_unicode_canon_decomp_index,
                                                       ...
                                                       n00b_unicode_canon_decomp_data,
                                                       cp);
...
static int
get_compat_decomp(n00b_codepoint_t cp, n00b_codepoint_t *out)
{
    ...
    const uint32_t *entry = n00b_unicode_sparse_lookup(n00b_unicode_compat_decomp_index,
                                                       ...
                                                       n00b_unicode_compat_decomp_data,
                                                       cp);
...
compose_pair(n00b_codepoint_t a, n00b_codepoint_t b)
{
...
static void
cp_buf_push(cp_buf_t *buf, n00b_codepoint_t cp)
{
    if (buf->len >= buf->cap) {
        buf->cap *= 2;
        {
            n00b_codepoint_t *new_data
                = n00b_alloc_array(char, buf->cap * sizeof(n00b_codepoint_t));
...
static void
decompose_recursive(n00b_codepoint_t cp, cp_buf_t *buf, bool compat)
{
...
static void
canonical_order(n00b_codepoint_t *cps, uint32_t len)
{
...
static void
canonical_compose(cp_buf_t *buf)
{
    if (buf->len < 2)
        return;
...
static cp_buf_t
decompose_str(const char *data, int64_t len, bool compat)
{
    cp_buf_t buf;
    cp_buf_init(&buf);
...
nqc_lookup(n00b_codepoint_t cp)
{
    if (cp >= 0x110000)
        return 0;
...
bool
n00b_unicode_is_nfc_raw(const char *data, int64_t len)
{
    uint32_t pos      = 0;
    uint8_t  last_ccc = 0;
...
bool
n00b_unicode_is_nfd_raw(const char *data, int64_t len)
{
    uint32_t pos      = 0;
    uint8_t  last_ccc = 0;
...
static void
normalizer_process(n00b_unicode_normalizer_t *n)
{
    if (n->pending.len == 0)
        return;
...
void
n00b_unicode_normalizer_free(n00b_unicode_normalizer_t *n)
{
    if (n) {
