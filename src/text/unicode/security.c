...
n00b_unicode_skeleton_raw(n00b_allocator_t *allocator, const char *data, int64_t len)
{
    ...
    while (pos < (uint32_t)nfd->u8_bytes) {
        ...
        const uint32_t *entry = n00b_unicode_sparse_lookup(n00b_unicode_confusable_index,
                                                           n00b_unicode_confusable_index_len,
                                                           n00b_unicode_confusable_data,
                                                           (n00b_codepoint_t)cp);
...
static bool
is_recommended(n00b_unicode_script_t sc)
{
...
static bool
is_highly_restrictive(const bool *scripts)
{
    ...
    static const n00b_unicode_script_t japanese[]
        = {SCRIPT_LATIN, SCRIPT_HAN, SCRIPT_HIRAGANA, SCRIPT_KATAKANA};
    static const n00b_unicode_script_t korean[]  = {SCRIPT_LATIN, SCRIPT_HAN, SCRIPT_HANGUL};
    static const n00b_unicode_script_t chinese[] = {SCRIPT_LATIN, SCRIPT_HAN, SCRIPT_BOPOMOFO};
...
static int
augmented_script_set(n00b_codepoint_t cp, n00b_unicode_script_t *out, int max)
{
...
n00b_unicode_script_restriction_raw(const char *data, int64_t len)
{
    ...
    while (pos < (uint32_t)len) {
        ...
        if ((n00b_codepoint_t)cp > 0x7F)
            ascii_only = false;
        if (all_id_allowed && !n00b_unicode_is_identifier_allowed((n00b_codepoint_t)cp))
...
bool
n00b_unicode_has_mixed_scripts_raw(const char *data, int64_t len)
{
