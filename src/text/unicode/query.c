...
/* These live in gen_names.c (auto-generated by gen_tables.py). */
extern const char *_n00b_gen_unicode_cp_name(uint32_t cp);
extern int         _n00b_gen_unicode_name_lookup(const char *name,
                                                 uint32_t   *out_cp);

n00b_option_t(const char *)
n00b_unicode_cp_name(n00b_codepoint_t cp)
{
    const char *name = _n00b_gen_unicode_cp_name((uint32_t)cp);
    if (!name) {
        return n00b_option_none(const char *);
    }
    return n00b_option_set(const char *, name);
}

n00b_option_t(n00b_codepoint_t) n00b_unicode_cp_from_name(const char *name)
{
    uint32_t cp;
    if (!_n00b_gen_unicode_name_lookup(name, &cp)) {
