...
dbg_dump_node(n00b_regex_builder_t *b, uint32_t id, int depth)
{
    ...
    if (id < N00B_RE_SENTINEL_COUNT) {
        const char *names[]
            = {"NOTHING", "EPSILON", "ANY", "DOTSTAR", "ANYPLUS", "BEGIN", "END"};
        fprintf(stderr, "[%u] %s\n", id, names[id]);
        return;
    }
    ...
    case N00B_RE_LOOP:
        fprintf(stderr,
                "[%u] Loop(lo=%d, hi=%d) nullable=%d always=%d\n",
                id,
                n->loop.lo,
                n->loop.hi,
                n->can_be_nullable,
                n->is_always_nullable);
        dbg_dump_node(b, n->loop.body, depth + 1);
        break;
    case N00B_RE_CONCAT:
        fprintf(stderr,
                "[%u] Concat nullable=%d always=%d\n",
                id,
                n->can_be_nullable,
                n->is_always_nullable);
    ...
    case N00B_RE_LOOKAROUND:
        fprintf(stderr,
                "[%u] Look%s(rel=%d, n_pending=%u) nullable=%d always=%d\n",
                id,
                n->lookaround.look_back ? "behind" : "ahead",
                n->lookaround.relative_to,
                n->lookaround.n_pending,
                n->can_be_nullable,
                n->is_always_nullable);
...
extern uint32_t
n00b_regex_dfa_step(n00b_regex_dfa_t *dfa, uint32_t state_id, n00b_regex_minterm_id_t mt_id);

extern uint32_t n00b_regex_dfa_step_end(n00b_regex_dfa_t       *dfa,
                                        uint32_t                state_id,
                                        n00b_regex_minterm_id_t mt_id);

extern bool n00b_regex_node_is_nullable(n00b_regex_builder_t *b, uint32_t node_id, int loc);
...
static int64_t accel_find_prefix(const char                     *data,
                                 uint32_t                        data_len,
        ...
        if (st->pending_ranges && st->n_pending > 0) {
            ...
            for (int32_t i = (int32_t)st->n_pending - 1; i >= 0; i--) {
                for (int32_t j = (int32_t)st->pending_ranges[i].hi;
                     j >= (int32_t)st->pending_ranges[i].lo;
                     j--) {
                    acc_push(acc, j + pos);
                }
            }
...
utf8_decode_backward(const char *data, int64_t pos)
{
    ...
    return (backward_decode_t){.cp       = (n00b_codepoint_t)(cp >= 0 ? cp : 0xFFFD),
                               .prev_pos = prev};
}
                 ...
                 acc_t            *acc)
{
    ...
    if (!nullable_at_end && st->node_id >= N00B_RE_SENTINEL_COUNT) {
        nullable_at_end = n00b_regex_node_is_nullable(rev_dfa->builder, st->node_id, LOC_END);
    }
    ...
    if (!hie_dbg_checked) {
        hie_dbg         = getenv("N00B_RE_DEBUG") != nullptr;
        hie_dbg_checked = true;
    }
    if (hie_dbg) {
        fprintf(stderr,
                "DBG HIE: state=%u node=%u flags=0x%x null_kind=%u nullable_at_end=%d "
                "depends_on_anchor=%d pos=%" PRId64 "\n",
                *state_id,
                st->node_id,
                st->flags,
                st->null_kind,
    ...
    if (nullable_at_end) {
        ...
        if (hie_dbg)
            fprintf(stderr, "DBG HIE: added pos=%" PRId64 "\n", pos);
    }

    // Take one End-location transition backward from the last codepoint
    if (pos > 0) {
        n00b_regex_state_flags_t initial_flags = st->flags;
        backward_decode_t        bd            = utf8_decode_backward(data, pos);
        n00b_regex_minterm_id_t  mt = n00b_regex_minterm_classify(rev_dfa->minterms, bd.cp);
        *state_id                   = n00b_regex_dfa_step_end(rev_dfa, *state_id, mt);
        ...
        if (hie_dbg) {
            fprintf(
                stderr,
                "DBG HIE: end-trans -> state=%u node=%u null_kind=%u flags=0x%x pos=%" PRId64
                "\n",
                *state_id,
                st->node_id,
                st->null_kind,
        ...
        if (pos != 0) {
            if (state_is_null(st)) {
                ...
                if (hie_dbg)
                    fprintf(stderr, "DBG HIE: added null pos=%" PRId64 "\n", pos);
            }
        }
        else {
            bool nullable_at_begin = (initial_flags & N00B_RE_SF_ALWAYS_NULLABLE) != 0;
            if (!nullable_at_begin && st->node_id >= N00B_RE_SENTINEL_COUNT) {
                nullable_at_begin
                    = n00b_regex_node_is_nullable(rev_dfa->builder, st->node_id, LOC_BEGIN);
            }
            if (nullable_at_begin) {
                if (state_is_null(st)) {
                    add_null_positions(st, acc, pos);
                    if (st->null_kind == N00B_RE_NULL_PREV) {
                        acc_push(acc, 0);
                    }
                }
                else if (st->pending_ranges && st->n_pending > 0) {
                    add_pending_rev(st, acc, pos);
                }
                else {
                    acc_push(acc, 0);
                ...
                if (hie_dbg)
                    fprintf(stderr, "DBG HIE: added begin pos=0\n");
            }
        }
    }
...
static void
handle_input_start(n00b_regex_dfa_t *rev_dfa, uint32_t state_id, acc_t *acc)
{
    n00b_regex_dfa_state_t *st = &rev_dfa->states.data[state_id];
    ...
    if (acc->len > 0 && acc->data[acc->len - 1] == 0)
        return;
               ...
               int64_t           start_pos)
{
    ...
    if (!cs_dbg_checked) {
        cs_dbg         = getenv("N00B_RE_DEBUG") != nullptr;
        cs_dbg_checked = true;
    }
    if (cs_dbg) {
        fprintf(stderr, "DBG CS: init_state=%u start_pos=%" PRId64 "\n", init_state, start_pos);
    }
    ...
    if (rev_dfa->is_flat) {
        ...
        while (pos > 0) {
            ...
            if (cs_dbg) {
                fprintf(stderr,
                        "DBG CS: pos=%" PRId64 "->%" PRId64
                        " char='%c' mt=%u: state %u->%u null_kind=%u flags=0x%x\n",
                        pos,
                        prev_pos,
                        prev_byte,
                        mt,
            ...
            if (state_is_null(st)) {
                ...
                if (cs_dbg)
                    fprintf(stderr, "DBG CS: added null prev_pos=%" PRId64 "\n", prev_pos);
            }
            pos = prev_pos;
        }
    }
...
static int64_t
reverse_find_start(n00b_regex_t *re,
                   const char   *data,
                   uint32_t      data_len,
                   int64_t       end_pos,
                   int64_t       lower_bound)
{
...

    {
        ...
        if (!rfs_dbg_checked) {
            rfs_dbg         = getenv("N00B_RE_DEBUG") != nullptr;
            rfs_dbg_checked = true;
        }
        if (rfs_dbg && data_len <= 20) {
            fprintf(stderr,
                    "DBG RFS: end_pos=%" PRId64 " lower=%" PRId64 " leftmost=%" PRId64
                    " state=%u pos=%" PRId64 "\n",
                    end_pos,
                    lower_bound,
                    leftmost,
                     ...
                     uint32_t          init_state)
{
    ...
    if (!fl_dbg_checked) {
        fl_dbg         = getenv("N00B_RE_DEBUG") != nullptr;
        fl_dbg_checked = true;
    }
    ...
    if (fl_dbg) {
        fprintf(stderr,
                "DBG FWD: start=%u init_state=%u node=%u flags=0x%x null_kind=%u\n",
                start,
                init_state,
                st->node_id,
                st->flags,
    ...
    if (fwd_dfa->is_flat) {
        ...
        while (pos < data_len) {
            ...
            if (fl_dbg) {
                fprintf(stderr,
                        "DBG FWD: pos=%u char='%c' mt=%u: state %u->%u node=%u null_kind=%u "
                        "flags=0x%x\n",
                        pos,
                        b,
                        mt,
...
fwd_done:
    ...
    if (pos >= data_len) {
        st = &fwd_dfa->states.data[state];
        if (st->flags
            & (N00B_RE_SF_END_NULLABLE | N00B_RE_SF_ALWAYS_NULLABLE
    ...
    if (fl_dbg) {
        fprintf(stderr,
                "DBG FWD: returning last_final=%" PRId64 " pos=%u state=%u\n",
                last_final,
                pos,
                state);
    }
...
static int64_t
forward_longest(n00b_regex_dfa_t *fwd_dfa, const char *data, uint32_t data_len, uint32_t start)
{
...
static uint32_t
find_all_matches(n00b_regex_t  *re,
                 const char    *data,
                 uint32_t       data_len,
                 match_pair_t **out_matches)
{
    ...
    if (!fam_dbg_checked) {
        fam_dbg         = getenv("N00B_RE_DEBUG") != nullptr;
        fam_dbg_checked = true;
    }
    if (fam_dbg) {
        fprintf(stderr,
                "DBG FAM: pat='%.*s' data_len=%u has_anchors=%d accel=%d\n",
    ...
    if (re->optimizations.override_.kind == N00B_RE_OVERRIDE_FIXED_STRING && !re->has_anchors) {
    ...
    bool accel_can_preserve_start         = accel->kind != N00B_RE_ACCEL_SINGLE_MINTERM
                                 && accel->kind != N00B_RE_ACCEL_POTENTIAL_START;

    if (accel->kind != N00B_RE_ACCEL_NONE && !re->has_anchors && accel_can_preserve_start) {
        ...
        while (search_start < data_len) {
            ...
            if (fam_dbg && data_len <= 20) {
                fprintf(stderr,
                        "DBG ACCEL: found=%" PRId64 " end=%" PRId64
                        " ts=%u plen=%u needs_rev=%d\n",
                        found,
                        end,
                        ts,
                        plen,
                        accel->needs_reverse_start);
            }
            ...
            if (end >= 0) {
                ...
                if (accel->needs_reverse_start) {
                    ...
                    int64_t rev_start
                        = reverse_find_start(re, data, data_len, end, (int64_t)search_start);
                    if (rev_start >= 0) {
    ...
    int64_t scan_start = handle_input_end(re->reverse_dfa, data, data_len, &rev_state, &acc);
    ...
    if (!dbg_checked) {
        dbg         = getenv("N00B_RE_DEBUG") != nullptr;
        dbg_checked = true;
    }
    if (dbg) {
        fprintf(stderr,
                "DBG three-phase: data_len=%u scan_start=%" PRId64 " acc.len=%u rev_state=%u\n",
                data_len,
                scan_start,
                acc.len,
                rev_state);
        for (uint32_t di = 0; di < acc.len; di++)
            fprintf(stderr, "  acc[%u] = %" PRId64 "\n", di, acc.data[di]);
    }
    ...
    for (int64_t i = (int64_t)acc.len - 1; i >= 0; i--) {
        ...
        if (dbg) {
            fprintf(stderr,
                    "DBG P3: start=%" PRId64 " end=%" PRId64 " next_valid=%" PRId64 "\n",
                    s,
                    end,
                    next_valid);
        }
...
static int64_t
fixed_string_find(const char             *data,
                  uint32_t                data_len,
                  const n00b_codepoint_t *needle,
                  uint32_t                needle_len,
                  uint32_t                start)
{
    ...
    while (pos + needle_bytes <= data_len) {
        const char *found
            = memchr(data + pos, needle_utf8[0], data_len - pos - needle_bytes + 1);
        if (!found)
            return -1;
        pos = (uint32_t)(found - data);
        if (pos + needle_bytes > data_len)
            return -1;
...
static int64_t
accel_find_prefix(const char                     *data,
                  uint32_t                        data_len,
                  const n00b_regex_accelerator_t *accel,
    ...
    case N00B_RE_ACCEL_FIXED_PREFIX_CI: {
        ...
        if (all_ascii && !ci) {
            ...
            while (pos + prefix_bytes <= data_len) {
                ...
                const char *found
                    = memchr(data + pos, prefix_utf8[0], data_len - pos - prefix_bytes + 1);
                if (!found)
                    return -1;
                uint32_t fpos = (uint32_t)(found - data);
                if (fpos + prefix_bytes > data_len)
                    return -1;
        ...
        if (all_ascii && ci) {
            ...
            char first_upper
                = (first_lower >= 'a' && first_lower <= 'z') ? first_lower - 32 : first_lower;
            uint32_t pos = start;
            while (pos + prefix_bytes <= data_len) {
                ...
                const char *fu        = (first_lower != first_upper)
                                          ? memchr(data + pos, first_upper, remaining)
                                          : nullptr;
                uint32_t    best      = data_len;
                if (fl)
                    best = (uint32_t)(fl - data);
                if (fu && (uint32_t)(fu - data) < best)
                    best = (uint32_t)(fu - data);
                if (best + prefix_bytes > data_len)
                    return -1;
        ...
        for (uint32_t pos = start; pos < data_len;) {
            ...
            if (b < 0x80) {
                ...
                if (cp < 0)
                    break;
                first_hit
                    = n00b_regex_charset_contains(solver, minterms[0], (n00b_codepoint_t)cp);
            }
            ...
            if (first_hit) {
                ...
                for (uint32_t i = 1; i < plen; i++) {
                    ...
                    if (!n00b_regex_charset_contains(solver,
                                                     minterms[i],
                                                     (n00b_codepoint_t)c2)) {
                        full_match = false;
                        break;
                  ...
                  int64_t                        match_start)
{
    ...
    case N00B_RE_LEN_SET_LOOKUP: {
        ...
        if (pos < data_len) {
            ...
            int32_t  cp    = n00b_unicode_utf8_decode(data, data_len, &pos);
            if (cp >= 0
                && n00b_regex_charset_contains(solver,
                                               len->set_lookup.minterm,
                                               (n00b_codepoint_t)cp)) {
                return (int64_t)pos;
            }
            (void)saved;
    ...
    case N00B_RE_LEN_FIXED_PREFIX_MATCH_END: {
        ...
        if (ts == 0)
            return -1; // No valid transition state — full DFA needed
...
n00b_regex_is_match(n00b_regex_t *re, n00b_string_t *input)
{
    ...
    if (re->optimizations.override_.kind == N00B_RE_OVERRIDE_FIXED_STRING) {
        return fixed_string_find(data,
                                 data_len,
                                 re->optimizations.override_.fixed_string.codepoints,
                                 re->optimizations.override_.fixed_string.len,
                                 0)
            >= 0;
    }
    ...
    if (re->reverse_dfa->is_flat) {
        ...
        while (pos > 0) {
            ...
            if (state_is_null(st)) {
                int64_t end
                    = forward_longest(re->forward_dfa, data, data_len, (uint32_t)prev_pos);
                if (end >= 0) {
                    acc_free(&acc);
                    return true;
                }
    ...
    for (int64_t i = (int64_t)acc.len - 1; i >= 0; i--) {
        int64_t end = forward_longest(re->forward_dfa, data, data_len, (uint32_t)acc.data[i]);
        if (end >= 0) {
            acc_free(&acc);
            return true;
        }
...
n00b_list_t(n00b_regex_match_t) * n00b_regex_matches(n00b_regex_t *re, n00b_string_t *input)
{
    ...
    *result                                 = n00b_list_new_private(n00b_regex_match_t);
    ...
    for (uint32_t i = 0; i < n; i++) {
        n00b_regex_match_t m
            = {.index = matches[i].start, .length = matches[i].end - matches[i].start};
...
n00b_string_t *
n00b_regex_replace(n00b_regex_t *re, n00b_string_t *input, n00b_string_t *replacement)
{
    const char *data     = input->data;
    uint32_t    data_len = (uint32_t)input->u8_bytes;
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (s > (int64_t)pos) {
            n00b_buffer_t *seg = n00b_buffer_from_bytes((char *)(data + pos),
                                                        s - (int64_t)pos,
                                                        .no_lock = true);
        ...
        if (has_dollar_zero) {
            ...
            for (int64_t j = 0; j < repl_len; j++) {
                if (j < repl_len - 1 && repl_data[j] == '$' && repl_data[j + 1] == '0') {
                    if (e > s) {
                        n00b_buffer_t *mseg = n00b_buffer_from_bytes((char *)(data + s),
                                                                     e - s,
                                                                     .no_lock = true);
                    ...
                    n00b_buffer_t *ch
                        = n00b_buffer_from_bytes((char *)&repl_data[j], 1, .no_lock = true);
            ...
            if (repl_len > 0) {
                n00b_buffer_t *rseg
                    = n00b_buffer_from_bytes((char *)repl_data, repl_len, .no_lock = true);
    ...
    if (pos < data_len) {
        n00b_buffer_t *tail
            = n00b_buffer_from_bytes((char *)(data + pos), data_len - pos, .no_lock = true);
...
n00b_list_t(n00b_string_t *) * n00b_regex_split(n00b_regex_t *re, n00b_string_t *input)
{
