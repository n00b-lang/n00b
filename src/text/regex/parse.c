...
charset_for_cp(parse_ctx_t *p, n00b_codepoint_t cp)
{
    ...
    if (p->case_insensitive) {
        ...
        if (folded != cp) {
            cs = n00b_regex_charset_or(p->solver,
                                       cs,
...
charset_word(parse_ctx_t *p)
{
    ...
    for (int i = 0; i < (int)(sizeof(word_cats) / sizeof(word_cats[0])); i++) {
        w = n00b_regex_charset_or(p->solver,
                                  w,
...
charset_space(parse_ctx_t *p)
{
    ...
    n00b_codepoint_t     ws[] = {' ', '\t', '\n', '\r', '\f', '\v'};
    for (int i = 0; i < 6; i++) {
        s = n00b_regex_charset_or(p->solver, s, n00b_regex_charset_single(p->solver, ws[i]));
    }
    // Also add Unicode Zs category
    s = n00b_regex_charset_or(p->solver,
                              s,
...
parse_unicode_property(parse_ctx_t *p, bool negated)
{
    ...
    static const gc_entry_t gc_table[] = {
        {"Lu", N00B_UNICODE_GC_LU}, {"Ll", N00B_UNICODE_GC_LL}, {"Lt", N00B_UNICODE_GC_LT},
        {"Lm", N00B_UNICODE_GC_LM}, {"Lo", N00B_UNICODE_GC_LO}, {"Mn", N00B_UNICODE_GC_MN},
        {"Mc", N00B_UNICODE_GC_MC}, {"Me", N00B_UNICODE_GC_ME}, {"Nd", N00B_UNICODE_GC_ND},
        {"Nl", N00B_UNICODE_GC_NL}, {"No", N00B_UNICODE_GC_NO}, {"Pc", N00B_UNICODE_GC_PC},
        {"Pd", N00B_UNICODE_GC_PD}, {"Ps", N00B_UNICODE_GC_PS}, {"Pe", N00B_UNICODE_GC_PE},
        {"Pi", N00B_UNICODE_GC_PI}, {"Pf", N00B_UNICODE_GC_PF}, {"Po", N00B_UNICODE_GC_PO},
        {"Sm", N00B_UNICODE_GC_SM}, {"Sc", N00B_UNICODE_GC_SC}, {"Sk", N00B_UNICODE_GC_SK},
        {"So", N00B_UNICODE_GC_SO}, {"Zs", N00B_UNICODE_GC_ZS}, {"Zl", N00B_UNICODE_GC_ZL},
        {"Zp", N00B_UNICODE_GC_ZP}, {"Cc", N00B_UNICODE_GC_CC}, {"Cf", N00B_UNICODE_GC_CF},
        {"Cs", N00B_UNICODE_GC_CS}, {"Co", N00B_UNICODE_GC_CO}, {"Cn", N00B_UNICODE_GC_CN},
    };
    ...
    if (name_len == 1) {
        typedef struct {
            char              ch;
            n00b_unicode_gc_t first;
            n00b_unicode_gc_t last;
        } super_t;
        ...
        for (int i = 0; i < (int)(sizeof(supers) / sizeof(supers[0])); i++) {
            if (name[0] == supers[i].ch) {
                for (int g = (int)supers[i].first; g <= (int)supers[i].last; g++) {
                    result = n00b_regex_charset_or(
                        p->solver,
                        result,
...
parse_escape_class(parse_ctx_t *p)
{
    ...
    switch (c) {
    case 'd':
        return charset_digit(p);
    case 'D':
        return n00b_regex_charset_not(p->solver, charset_digit(p));
    case 'w':
        return charset_word(p);
    case 'W':
        return n00b_regex_charset_not(p->solver, charset_word(p));
    case 's':
        return charset_space(p);
    case 'S':
        return n00b_regex_charset_not(p->solver, charset_space(p));
    case 'p':
        return parse_unicode_property(p, false);
    case 'P':
        return parse_unicode_property(p, true);
    case 'n':
        return charset_for_cp(p, '\n');
    case 'r':
        return charset_for_cp(p, '\r');
    case 't':
        return charset_for_cp(p, '\t');
    case 'f':
        return charset_for_cp(p, '\f');
    case 'v':
        return charset_for_cp(p, '\v');
    case 'a':
        return charset_for_cp(p, '\a');
    case 'x': {
        // \xHH
        uint32_t val = 0;
...
classify_right_edge(parse_ctx_t *p, uint32_t node_id)
{
    ...
    if (n->kind == N00B_RE_LOOP)
        return classify_right_edge(p, n->loop.body);
    if (n->kind == N00B_RE_CONCAT)
        return classify_right_edge(p, n->concat.tail);
    return WCK_UNKNOWN;
}
...
classify_left_edge(parse_ctx_t *p, uint32_t node_id)
{
    ...
    n00b_regex_node_t *n = n00b_regex_node_get(p->builder, node_id);
    if (n->kind == N00B_RE_SINGLETON)
        return classify_right_edge(p, node_id);
    if (n->kind == N00B_RE_LOOP)
        return classify_left_edge(p, n->loop.body);
    if (n->kind == N00B_RE_CONCAT)
        return classify_left_edge(p, n->concat.head);
    return WCK_UNKNOWN;
}

static bool
singleton_set(parse_ctx_t *p, uint32_t node_id, n00b_regex_charset_t *out)
{
    if (node_id >= p->builder->nodes.len) {
        return false;
    }
    n00b_regex_node_t *n = n00b_regex_node_get(p->builder, node_id);
    if (n->kind != N00B_RE_SINGLETON) {
...
static bool
lookahead_first_set(parse_ctx_t *p, uint32_t node_id, n00b_regex_charset_t *out)
{
    if (node_id >= p->builder->nodes.len) {
        return false;
    }
    n00b_regex_node_t *n = n00b_regex_node_get(p->builder, node_id);
    if (n->kind != N00B_RE_LOOKAROUND || n->lookaround.look_back) {
        return false;
    }

    uint32_t body = n->lookaround.body;
    if (body >= p->builder->nodes.len) {
        return false;
    }
    if (singleton_set(p, body, out)) {
        return true;
    }

    n00b_regex_node_t *body_n = n00b_regex_node_get(p->builder, body);
    if (body_n->kind == N00B_RE_CONCAT && body_n->concat.tail == N00B_RE_ID_DOTSTAR) {
        return singleton_set(p, body_n->concat.head, out);
    }
    return false;
}

static bool
lookbehind_last_set(parse_ctx_t *p, uint32_t node_id, n00b_regex_charset_t *out)
{
    if (node_id >= p->builder->nodes.len) {
        return false;
    }
    ...
    n00b_regex_node_t *body_n = n00b_regex_node_get(p->builder, body);
    if (body_n->kind == N00B_RE_CONCAT && body_n->concat.head == N00B_RE_ID_DOTSTAR) {
        return singleton_set(p, body_n->concat.tail, out);
    }
    return false;
}
 ...
 */
static uint32_t
expand_word_boundary(parse_ctx_t     *p,
                     bool             is_boundary,
                     ...
                     word_char_kind_t right)
{
    ...
    uint32_t word_left
        = n00b_regex_mk_lookaround(b,
                                   n00b_regex_mk_or2(b, N00B_RE_ID_BEGIN, word_node),
                                   true,
                                   0,
                                   nullptr,
                                   0);
    uint32_t nonword_left
        = n00b_regex_mk_lookaround(b,
                                   n00b_regex_mk_or2(b, N00B_RE_ID_BEGIN, nonword_node),
                                   true,
                                   0,
                                   nullptr,
                                   0);
    uint32_t word_right
        = n00b_regex_mk_lookaround(b,
                                   n00b_regex_mk_or2(b, N00B_RE_ID_END, word_node),
                                   false,
                                   0,
                                   nullptr,
                                   0);
    uint32_t nonword_right
        = n00b_regex_mk_lookaround(b,
                                   n00b_regex_mk_or2(b, N00B_RE_ID_END, nonword_node),
                                   false,
                                   0,
                                   nullptr,
                                   0);
    ...
    if (is_boundary) {
        ...
        if ((left == WCK_WORD && right == WCK_WORD)
            ...
            return N00B_RE_ID_NOTHING;
        if (left == WCK_WORD)
            return nonword_right;
        if (left == WCK_NONWORD)
            return word_right;
        if (right == WCK_WORD)
            return nonword_left;
        if (right == WCK_NONWORD)
            return word_left;
        ...
        return n00b_regex_mk_or2(b, n00b_regex_mk_and(b, c1, 2), n00b_regex_mk_and(b, c2, 2));
    }
    else {
...
parse_group(parse_ctx_t *p)
{
    ...
    if (match_char(p, '?')) {
        if (match_char(p, ':')) {
            ...
            return n00b_regex_mk_lookaround(p->builder,
                                            n00b_result_get(neg_r),
                                            false,
                                            0,
                                            nullptr,
                                            0);
            ...
            if (match_char(p, '=')) {
                ...
                n00b_regex_node_t *body_node = n00b_regex_node_get(p->builder, body);
                if (body_node->kind == N00B_RE_SINGLETON) {
                    n00b_regex_charset_t neg
                        = n00b_regex_charset_not(p->solver, body_node->singleton.set);
                    uint32_t neg_node = n00b_regex_mk_singleton(p->builder, neg);
                    uint32_t look
                        = n00b_regex_mk_lookaround(p->builder, neg_node, true, 0, nullptr, 0);
                    return n00b_regex_mk_or2(p->builder, N00B_RE_ID_BEGIN, look);
                }
                uint32_t wrapped = n00b_regex_mk_concat(p->builder, N00B_RE_ID_DOTSTAR, body);
...
parse_atom(parse_ctx_t *p)
{
    ...
    case '^':
        ...
        if (p->multiline) {
            ...
            uint32_t body = n00b_regex_mk_or2(
                p->builder,
                N00B_RE_ID_BEGIN,
...
parse_sequence(parse_ctx_t *p)
{
    ...
    for (uint32_t i = 0; i < n_terms; i++) {
        if (terms[i] != WB_MARKER && terms[i] != NWB_MARKER)
            continue;
        ...
        if (i == n_terms - 1) {
            ...
            uint32_t j = i + 1;
            while (j < n_terms && (terms[j] == WB_MARKER || terms[j] == NWB_MARKER))
                j++;
            if (j < n_terms) {
    ...
    for (uint32_t i = 0; i < n_terms; i++) {
        n00b_regex_charset_t assert_set;
        n00b_regex_charset_t consume_set;

        if (i + 1 < n_terms && lookahead_first_set(p, terms[i], &assert_set)
            && singleton_set(p, terms[i + 1], &consume_set)) {
            n00b_regex_charset_t narrowed
                = n00b_regex_charset_and(p->solver, consume_set, assert_set);
            terms[i]     = N00B_RE_ID_EPSILON;
            terms[i + 1] = n00b_regex_mk_singleton(p->builder, narrowed);
            continue;
        }

        if (i > 0 && lookbehind_last_set(p, terms[i], &assert_set)
            && singleton_set(p, terms[i - 1], &consume_set)) {
            n00b_regex_charset_t narrowed
                = n00b_regex_charset_and(p->solver, consume_set, assert_set);
            terms[i - 1] = n00b_regex_mk_singleton(p->builder, narrowed);
...
n00b_result_t(uint32_t) n00b_regex_parse(n00b_regex_builder_t *builder,
