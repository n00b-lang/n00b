...
// libchalk shim around n00b's SHA-256: keeps the SHA256() call
// signature the original codec used so the body changes are minimal.
static unsigned char *
chalk_gguf_sha256(const unsigned char *d, size_t n, unsigned char *md)
{
...
#define SHA256(d, n, md) chalk_gguf_sha256((d), (n), (md))
...
// Diagnostic sink. libchalk drops chalk's libc-based fprintf;
// callers wire this to n00b's logging if they care.
[[gnu::weak]]
...
extern chalk_gguf_t *chalk_gguf_parse(const uint8_t *bytes, size_t length) {
    ...
    chalk_gguf_t *g = n00b_alloc(chalk_gguf_t);
    if (!g) {
        return NULL;
    }
...
static chalk_gguf_status_t rebuild(chalk_gguf_t *g,
    ...
    uint8_t *out = (uint8_t *)n00b_alloc_array(char, new_total);
    if (!out) {
...
extern chalk_gguf_status_t chalk_gguf_unchalked_hash(chalk_gguf_t *g,
    ...
    if (!g->has_chalk) {
        ...
        canon     = (uint8_t *)n00b_alloc_array(char, canon_len);
        if (!canon) {
            return CHALK_GGUF_ERR_INTERNAL;
        }
