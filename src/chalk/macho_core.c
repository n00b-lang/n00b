...
// ============================================================================
// SHA-256 via n00b. The shim preserves the OpenSSL SHA256() signature
// so the original Mach-O code's call sites are untouched.
// ============================================================================

static unsigned char *
chalk_macho_sha256(const unsigned char *d, size_t n, unsigned char *md)
{
...
#define SHA256(d, n, md) chalk_macho_sha256((d), (n), (md))
...
{
    // libchalk drops chalk's libc-based varargs formatter. We pass
    // the bare format string through; callers wire chalk_macho_warn
    // to n00b's logger if they want richer diagnostics.
    chalk_macho_warn(fmt);
}
...
chalk_macho_strip_signature(macho_binary_t *bin)
{
    ...
    (void)(bin->commands[sig_idx].raw_data);
...
chalk_macho_remove_note(macho_binary_t *bin)
{
    ...
    // Sync bin->commands[].
    (void)(bin->commands[idx].raw_data);
