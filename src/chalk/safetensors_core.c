...
chalk_st_sha256(const unsigned char *d, size_t n, unsigned char *md)
{
    n00b_sha256_digest_t words;
    n00b_sha256_hash(d, n, words);
    for (int i = 0; i < 8; i++) {
        uint32_t w = words[i];
        md[i * 4]     = (uint8_t)((w >> 24) & 0xff);
        md[i * 4 + 1] = (uint8_t)((w >> 16) & 0xff);
        md[i * 4 + 2] = (uint8_t)((w >> 8) & 0xff);
        md[i * 4 + 3] = (uint8_t)(w & 0xff);
    }
    return md;
}
#define SHA256(d, n, md) chalk_st_sha256((d), (n), (md))
    ...
    for (size_t i = 0; i < in_len; i++) {
        ...
        default:
            if ((unsigned char)c < 0x20) {
                ...
                out[o++] = '0';
                out[o++] = '0';
                out[o++] = hexd[((unsigned char)c >> 4) & 0xf];
                out[o++] = hexd[(unsigned char)c & 0xf];
            } else {
                out[o++] = c;
            }
        }
...
extern chalk_st_t *chalk_st_parse(const uint8_t *bytes, size_t length) {
    ...
    chalk_st_t *st = n00b_alloc(chalk_st_t);
    if (!st) {
        return NULL;
    }
...
extern char *chalk_st_get_payload(chalk_st_t *st, size_t *out_size) {
    ...
    char  *out    = n00b_alloc_array(char, in_len + 1);
    if (!out) {
        return NULL;
    }
...
static chalk_st_status_t replace_header(chalk_st_t *st,
    ...
    uint8_t *fresh = (uint8_t *)n00b_alloc_array(char, new_total);
    if (!fresh) {
        return CHALK_ST_ERR_INTERNAL;
    }
...
extern chalk_st_status_t chalk_st_set_chalk(chalk_st_t *st,
    ...
    char  *escaped = n00b_alloc_array(char, mark_length * 2 + 1);
    if (!escaped) {
        return CHALK_ST_ERR_INTERNAL;
    }
    ...
    if (find_pair(p, hdr_end, META_KEY, META_KEY_LEN,
        ...
        // Equivalent to snprintf(..., "\"%s\":\"", CHALK_KEY).
        memcpy(out + op, "\"" CHALK_KEY "\":\"",
               1 + CHALK_KEY_LEN + 3);
        op += 1 + CHALK_KEY_LEN + 3;
        memcpy(out + op, escaped, escaped_len);
        op += escaped_len;
        out[op++] = '"';
        ...
        // Equivalent to snprintf(..., "\"%s\":{\"%s\":\"", META_KEY, CHALK_KEY).
        memcpy(out + op,
               "\"" META_KEY "\":{\"" CHALK_KEY "\":\"",
               1 + META_KEY_LEN + 4 + CHALK_KEY_LEN + 3);
        op += 1 + META_KEY_LEN + 4 + CHALK_KEY_LEN + 3;
        memcpy(out + op, escaped, escaped_len);
        op += escaped_len;
        out[op++] = '"';
        out[op++] = '}';
...
extern chalk_st_status_t chalk_st_remove_chalk(chalk_st_t *st) {
    ...
    char       *out     = n00b_alloc_array(char, new_len);
    if (!out) {
        return CHALK_ST_ERR_INTERNAL;
    }
...
extern chalk_st_status_t chalk_st_unchalked_hash(chalk_st_t *st,
    ...
    if (!has_chalk) {
        ...
        canon     = (unsigned char *)n00b_alloc_array(char, canon_len);
        if (!canon) {
            return CHALK_ST_ERR_INTERNAL;
        }
