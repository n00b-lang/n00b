...
// extract_first_identifier — now in tree_util.c as
// n00b_tree_extract_first_identifier().
#define extract_first_identifier n00b_tree_extract_first_identifier
...
// find_child_nt — now in tree_util.c as n00b_tree_find_child_by_nt_name().
#define find_child_nt n00b_tree_find_child_by_nt_name
    ...
    for (size_t i = 0; i < nc; i++) {
        ...
        for (size_t j = 0; j < cnc; j++) {
            ...
            if (n00b_tree_is_leaf(tcc)) {
                n00b_string_t s = extract_first_identifier(tcc);
        ...
        n00b_parse_tree_t *clist = find_child_nt(g, child, *r"constraint-list");
...
handle_constraint:;
            ...
            n00b_string_t first = extract_first_identifier(con_node);

            if (n00b_unicode_str_eq(first, *r"!=")) {
                // Exclusion constraint: != <one-tspec>
                n00b_parse_tree_t *excluded_node = find_child_nt(g, con_node,
                                                                    *r"one-tspec");
    ...
    *types_ptr = types;
    ...
    for (size_t i = 0; i < nc; i++) {
        ...
        if (cpn->group_top) {
            ...
            for (size_t j = 0; j < gnc; j++) {
                ...
                if (nt_name_is(g, gchild, *r"tspec-field")) {
                    n00b_string_t fname = extract_first_identifier(gchild);

                    if (fname.u8_bytes > 0) {
                        // Find the type child of the field.
                        n00b_parse_tree_t *ft = find_child_nt(g, gchild,
                                                                 *r"type-spec-body");

                        if (!ft) {
                            ft = find_child_nt(g, gchild, *r"one-tspec");
                        }
        ...
        if (nt_name_is(g, child, *r"tspec-field")) {
            n00b_string_t fname = extract_first_identifier(child);

            if (fname.u8_bytes > 0) {
                n00b_parse_tree_t *ft = find_child_nt(g, child,
                                                         *r"type-spec-body");

                if (!ft) {
                    ft = find_child_nt(g, child, *r"one-tspec");
                }
...
translate(n00b_tc_ctx_t *ctx, n00b_grammar_t *g, n00b_parse_tree_t *node)
{
    ...
    if (n00b_unicode_str_eq(nt->name, *r"type-spec")) {
        ...
        n00b_parse_tree_t *where_node = find_child_nt(g, node, *r"where-clause");
    ...
    if (n00b_unicode_str_eq(nt->name, *r"tspec-func")) {
        ...
        *pos_ptr = positional;
        ...
        if (func_params) {
            ...
            n00b_parse_tree_t *vargs_node = find_child_nt(g, func_params,
                                                             *r"tspec-vargs");

            if (vargs_node) {
                // Find the <one-tspec> or <type-spec-body> child.
                n00b_parse_tree_t *vt = find_child_nt(g, vargs_node,
                                                         *r"type-spec-body");

                if (!vt) {
                    vt = find_child_nt(g, vargs_node, *r"one-tspec");
                }
            ...
            n00b_parse_tree_t *kargs_node = find_child_nt(g, func_params,
                                                             *r"tspec-kargs");
