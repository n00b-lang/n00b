        ...
        if (a->kind == N00B_ANNOT_SCOPE_OPEN) {
            ...
            if (a->name_ref.kind == N00B_ROLE_BY_INDEX
                ...
                if (name_node) {
                    ...
                    if (resolved && resolved->u8_bytes > 0) {
                        scope_name = resolved;
                    }
                }
            }
            ...
            if (ctx->tc_ctx && a->scope_tag && a->scope_tag->u8_bytes > 0
            ...
            if (!scope_name || scope_name->u8_bytes == 0) {
                char buf[32];
            ...
            if (scope) {
                ...
                scope->adt_kind  = kind;
                scope->scope_tag = a->scope_tag ? a->scope_tag
                                                : a->adt_kind;
            }
