...
static uint8_t determine_paragraph_level(const n00b_codepoint_t *cps
                                         __attribute__((unused)),
                                         const uint8_t *types, uint32_t len) {
...
static void resolve_explicit(uint8_t *types, uint8_t *levels, uint32_t len,
                             uint8_t para_level) {
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (type == N00B_UNICODE_BIDI_RLE || type == N00B_UNICODE_BIDI_LRE ||
      ...
      if (new_level <= 125 && overflow_isolate == 0 &&
...
static void resolve_weak(uint8_t *types, const uint8_t *levels, uint32_t len) {
      ...
      if (next < len) {
        ...
        if (type == N00B_UNICODE_BIDI_ES && prev_type == N00B_UNICODE_BIDI_EN &&
          ...
          if ((prev_type == N00B_UNICODE_BIDI_EN && next_type == N00B_UNICODE_BIDI_EN) ||
              (prev_type == N00B_UNICODE_BIDI_AN && next_type == N00B_UNICODE_BIDI_AN)) {
            types[i] = prev_type;
          }
        }
      }
...
static int32_t find_bracket(n00b_codepoint_t cp) {
  int32_t lo = 0;
...
static bool get_bracket_info(n00b_codepoint_t cp,
                             n00b_codepoint_t *paired, int *bracket_type) {
...
static void resolve_brackets(const n00b_codepoint_t *cps, uint8_t *types,
                             const uint8_t *levels, uint32_t len,
                             uint8_t para_level __attribute__((unused))) {
...
static void resolve_neutral(uint8_t *types, const uint8_t *levels,
                            uint32_t len) {
  for (uint32_t i = 0; i < len; i++) {
    if (types[i] == N00B_UNICODE_BIDI_BN)
      continue;
...
static void resolve_implicit(const uint8_t *types, uint8_t *levels,
                             uint32_t len) {
  for (uint32_t i = 0; i < len; i++) {
    if (types[i] == N00B_UNICODE_BIDI_BN)
      continue;
...
static n00b_unicode_bidi_para_t *bidi_open_internal(const char *data, int64_t len) {
...
n00b_unicode_bidi_para_t *n00b_unicode_bidi_open(n00b_string_t s) _kargs { n00b_allocator_t *allocator = NULL; }
{
    (void)allocator;
