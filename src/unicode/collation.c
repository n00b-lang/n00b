...
static int32_t find_contraction_start(n00b_codepoint_t cp1) {
  int32_t lo = 0;
...
static int try_contraction(const n00b_codepoint_t *cps, const uint8_t *cccs,
                           uint32_t num_cps, uint32_t idx, uint16_t *out,
                           int max_ces, bool *matched_mask) {
...
// secondary, tertiary)
static int get_ces(n00b_codepoint_t cp, uint16_t *out, int max_ces) {
  uint32_t offset =
...
static n00b_unicode_sort_key_t sort_key_internal(const char *data, int64_t len) {
  ...
  for (uint32_t i = 0; i < num_cps; i++) {
    ...
    int n = try_contraction(cps, cccs, num_cps, i, ces + total_ces * 3,
                            remaining, matched);
    if (n > 0) {
      total_ces += n;
      continue;
...
n00b_unicode_sort_key_t n00b_unicode_sort_key(n00b_string_t s) _kargs { n00b_allocator_t *allocator = NULL; }
{
    (void)allocator;
    return sort_key_internal(s.data, s.u8_bytes);
}

static int collate_internal(const char *a, int64_t a_len, const char *b,
                            int64_t b_len) {
