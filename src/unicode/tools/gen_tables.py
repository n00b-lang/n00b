...
def download_file(url, dest):
    ...
    Returns:
        bool: True when the destination exists after the operation.
    """
    if dest.exists():
...
def download_all(version, cache_dir, test_data_dir):
    ...
    for f in UNICODE_FILES:
        ...
        if not download_file(url, dest):
            missing_required.append(dest.name)
    ...
    for f in TEST_FILES:
        ...
        if not download_file(url, cache_dir / f.replace("/", "_")):
            missing_required.append(f.replace("/", "_"))

    for name, url_template in SPECIAL_FILES.items():
        url = url_template.format(version=version)
        if not download_file(url, cache_dir / name) and name != "CollationTest.zip":
    ...
    if missing_required:
        uniq = ", ".join(sorted(set(missing_required)))
        raise RuntimeError(
            "missing required Unicode source files: "
            f"{uniq}. populate cache dir '{cache_dir}' or restore network access."
        )
...
def gen_collation(cache_dir, out_dir):
    ...
    def emit_empty_collation_tables():
        ...
        with open(out_dir / "gen_collation.c", "a") as f:
            f.write("\nconst uint16_t n00b_unicode_ce_data[1] = { 0x0000 };\n")
            f.write("const uint32_t n00b_unicode_ce_data_len = 0;\n\n")
            f.write("const uint32_t n00b_unicode_contraction_count = 0;\n")
            f.write("const uint32_t n00b_unicode_contr_keys[][4] = { { 0, 0, 0, 0 } };\n")
            f.write("const uint32_t n00b_unicode_contr_key_lens[] = { 0 };\n")
            f.write("const uint32_t n00b_unicode_contr_ce_offsets[] = { 0 };\n")
            f.write("const uint16_t n00b_unicode_contr_ce_data[1] = { 0x0000 };\n")

    path = cache_dir / "allkeys.txt"
    if not path.exists():
        print("  WARNING: allkeys.txt not found, skipping collation")
        emit_empty_collation_tables()
        return
