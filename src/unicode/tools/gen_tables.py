...
class TwoStageTable:
    ...
    def generate_c(self, out_path):
        ...
        with open(out_path, "w") as f:
            ...
            f.write(f'#include "unicode/types.h"\n\n')
...
def gen_scripts(cache_dir, out_dir):
    ...
    with open(out_dir / "gen_scripts.c", "a") as f:
        f.write(f"\nconst char *unicode_script_names[{len(SCRIPT_VALUES)}] = {{\n")
        for s in SCRIPT_VALUES:
            f.write(f'    "{s}",\n')
        f.write("};\n")
        f.write(f"const uint32_t unicode_script_count = {len(SCRIPT_VALUES)};\n")
...
def gen_script_extensions(cache_dir, out_dir):
    ...
    with open(out_dir / "gen_script_extensions.c", "w") as f:
        ...
        f.write('#include "unicode/types.h"\n\n')
        f.write(f"#define UNICODE_SCRIPT_COMMON {common_idx}\n")
        f.write(f"#define UNICODE_SCRIPT_INHERITED {inherited_idx}\n\n")
...
def gen_blocks(cache_dir, out_dir):
    ...
    with open(out_dir / "gen_blocks.c", "w") as f:
        ...
        f.write(f"const uint32_t unicode_block_ranges[][2] = {{\n")
        for start, end, name in blocks:
            f.write(f"    {{ 0x{start:06X}, 0x{end:06X} }},  // {name}\n")
        f.write("};\n")
        f.write(f"const uint16_t unicode_block_ids[] = {{\n")
        for start, end, name in blocks:
            f.write(f"    {BLOCK_MAP[name]},  // {name}\n")
        f.write("};\n")
        f.write(f"const uint32_t unicode_block_count = {len(blocks)};\n\n")

        f.write(f"const char *unicode_block_names[{len(BLOCK_VALUES)}] = {{\n")
...
def gen_age(cache_dir, out_dir):
    ...
    with open(out_dir / "gen_age.c", "a") as f:
        f.write(f"\nconst char *unicode_age_names[{len(age_list)}] = {{\n")
        for a in age_list:
            f.write(f'    "{a}",\n')
        f.write("};\n")
        f.write(f"const uint32_t unicode_age_count = {len(age_list)};\n")
...
def gen_composition(cache_dir, out_dir):
    ...
    with open(out_dir / "gen_composition.c", "w") as f:
        ...
        f.write(f"#define UNICODE_COMP_TABLE_SIZE {table_size}\n")
        f.write(f"#define UNICODE_COMP_EMPTY 0xFFFFFFFFU\n\n")

        # Store as parallel arrays: starter[], combining[], composed[]
        f.write(f"const uint32_t unicode_comp_starter[{table_size}] = {{\n")
        for i in range(0, table_size, 8):
            vals = [f"0x{keys[j][0]:08X}" for j in range(i, min(i + 8, table_size))]
            f.write("    " + ", ".join(vals) + ",\n")
        f.write("};\n\n")

        f.write(f"const uint32_t unicode_comp_combining[{table_size}] = {{\n")
        for i in range(0, table_size, 8):
            vals = [f"0x{keys[j][1]:08X}" for j in range(i, min(i + 8, table_size))]
            f.write("    " + ", ".join(vals) + ",\n")
        f.write("};\n\n")

        f.write(f"const uint32_t unicode_comp_result[{table_size}] = {{\n")
        for i in range(0, table_size, 8):
...
def gen_collation(cache_dir, out_dir):
    ...
    if not path.exists():
        ...
        with open(out_dir / "gen_collation.c", "w") as f:
            f.write("// Auto-generated by gen_tables.py — do not edit\n")
            f.write('#include "unicode/types.h"\n\n')
            f.write("const uint32_t unicode_ducet_count = 0;\n")
            f.write("const uint32_t unicode_contraction_count = 0;\n")
        return
    ...
    with open(out_dir / "gen_collation.c", "a") as f:
        ...
        if sorted_contr:
            ...
            f.write(f"const uint32_t unicode_contr_keys[][4] = {{\n")
            ...
            f.write(f"const uint32_t unicode_contr_key_lens[] = {{\n")
            for cps, offset in contr_index:
                f.write(f"    {len(cps)},\n")
            f.write("};\n\n")

            f.write(f"const uint32_t unicode_contr_ce_offsets[] = {{\n")
            for cps, offset in contr_index:
                f.write(f"    {offset},\n")
            f.write("};\n\n")

            f.write(f"const uint16_t unicode_contr_ce_data[{len(contr_data)}] = {{\n")
...
def main():
    ...
    parser.add_argument("--out-dir", default="src/generated",
