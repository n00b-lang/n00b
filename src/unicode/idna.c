...
static bool validate_contextj(const n00b_codepoint_t *cps, uint32_t len,
                              uint32_t pos) {
...
static bool validate_contexto(const n00b_codepoint_t *cps, uint32_t len,
                              uint32_t pos) {
  n00b_codepoint_t cp = cps[pos];
  ...
  case 0x00B7: // MIDDLE DOT
    ...
    return (pos > 0 && pos + 1 < len && cps[pos - 1] == 0x006C &&
            cps[pos + 1] == 0x006C);
...
static bool validate_label_context(const n00b_codepoint_t *cps, uint32_t len,
...
static uint8_t get_idna_status(n00b_codepoint_t cp) {
  if (cp >= 0x110000)
    return IDNA_DISALLOWED;
  return N00B_UNICODE_LOOKUP(n00b_unicode_idna_status_stage1, n00b_unicode_idna_status_stage2,
                        cp);
}
...
// Punycode encode. Returns malloc'd ASCII string or NULL on error.
static char *puny_encode(const n00b_codepoint_t *input, uint32_t input_len,
                         uint32_t *out_len) {
...
n00b_string_t n00b_unicode_idna_to_ascii_raw(n00b_allocator_t *allocator, const char *domain,
                                     int64_t len, n00b_unicode_idna_error_t *err) {
  if (err)
  ...
  while (pos < num_bytes) {
    ...
    case IDNA_MAPPED: {
      const uint32_t *entry = n00b_unicode_sparse_lookup(
          n00b_unicode_idna_map_index, n00b_unicode_idna_map_index_len,
          n00b_unicode_idna_map_data, (n00b_codepoint_t)cp);
      if (entry) {
...
n00b_unicode_idna_result_t n00b_unicode_idna_to_ascii(n00b_string_t domain) _kargs { n00b_allocator_t *allocator = NULL; }
{
    if (!allocator) allocator = nullptr;
    n00b_unicode_idna_error_t err = N00B_UNICODE_IDNA_OK;
    n00b_string_t value = n00b_unicode_idna_to_ascii_raw(allocator, domain.data, domain.u8_bytes, &err);
    return (n00b_unicode_idna_result_t){ .value = value, .error = err };
}

n00b_string_t n00b_unicode_idna_to_unicode_raw(n00b_allocator_t *allocator, const char *domain,
                                       int64_t len,
                                       n00b_unicode_idna_error_t *err) {
  if (err)
    *err = N00B_UNICODE_IDNA_OK;
...
n00b_unicode_idna_result_t n00b_unicode_idna_to_unicode(n00b_string_t domain) _kargs { n00b_allocator_t *allocator = NULL; }
{
    if (!allocator) allocator = nullptr;
    n00b_unicode_idna_error_t err = N00B_UNICODE_IDNA_OK;
    n00b_string_t value = n00b_unicode_idna_to_unicode_raw(allocator, domain.data, domain.u8_bytes, &err);
    return (n00b_unicode_idna_result_t){ .value = value, .error = err };
}
