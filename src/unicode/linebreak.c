...
static inline n00b_unicode_lb_t get_lb_class(n00b_codepoint_t cp) {
  if (cp >= 0x110000)
    return N00B_UNICODE_LB_XX;
  return (n00b_unicode_lb_t)N00B_UNICODE_LOOKUP(n00b_unicode_lb_stage1, n00b_unicode_lb_stage2, cp);
}

// Resolve AI, CJ, SA, XX to their effective classes
static n00b_unicode_lb_t resolve_lb(n00b_codepoint_t cp, n00b_unicode_lb_t cls) {
  switch (cls) {
...
static inline bool is_qu_pf(n00b_codepoint_t cp) {
  return n00b_unicode_general_category(cp) == N00B_UNICODE_GC_PF;
}

static inline bool is_qu_pi(n00b_codepoint_t cp) {
  return n00b_unicode_general_category(cp) == N00B_UNICODE_GC_PI;
}

static inline bool is_east_asian(n00b_codepoint_t cp) {
  n00b_unicode_eaw_t eaw = n00b_unicode_east_asian_width(cp);
  return (eaw == N00B_UNICODE_EAW_F || eaw == N00B_UNICODE_EAW_W || eaw == N00B_UNICODE_EAW_H);
}

// Is this a "Hyphen" for LB20.1? BA characters with GC=Pd that aren't East
// Asian
static inline bool is_hyphen_ba(n00b_codepoint_t cp, n00b_unicode_lb_t cls) {
...
static inline uint32_t find_base_idx(const n00b_codepoint_t *cps,
                                     uint32_t idx) {
  while (idx > 0) {
...
static inline bool is_break_context(n00b_unicode_lb_t cls) {
...
// as AL)
static inline bool is_aksara(n00b_unicode_lb_t cls, n00b_codepoint_t cp) {
...
void n00b_unicode_linebreaks_raw(const char *data, int64_t len,
  ...
  while (pos < num_bytes && n < (uint32_t)num_codepoints) {
    ...
    cls[n] = resolve_lb((n00b_codepoint_t)cp,
  ...
  for (uint32_t i = 1; i < n; i++) {
    ...
    if (cur == N00B_UNICODE_LB_AL &&
      ...
      bool is_hyp = (prev == N00B_UNICODE_LB_HY) ||
                    is_hyphen_ba(cps[prev_base], cls[prev_base]);
      if (is_hyp) {
    ...
    if (prev == N00B_UNICODE_LB_JL &&
        (cur == N00B_UNICODE_LB_JL || cur == N00B_UNICODE_LB_JV || cur == N00B_UNICODE_LB_H2 ||
    ...
    if (prev == N00B_UNICODE_LB_PR &&
        (cur == N00B_UNICODE_LB_JL || cur == N00B_UNICODE_LB_JV || cur == N00B_UNICODE_LB_JT ||
    ...
    if (prev == N00B_UNICODE_LB_CP &&
        !is_east_asian(cps[find_base_idx(cps, i - 1)]) &&
        (cur == N00B_UNICODE_LB_AL || cur == N00B_UNICODE_LB_HL ||
         cur == N00B_UNICODE_LB_NU)) {
      out[i] = N00B_UNICODE_LB_ACTION_NONE;
      continue;
...
uint32_t *n00b_unicode_linebreak_wrap_raw(const char *data, int64_t len, int width,
                                     uint32_t *num_breaks) {
  uint32_t num_bytes = (uint32_t)len;
  ...
  for (uint32_t i = 0; i < (uint32_t)num_codepoints; i++) {
    ...
    if (col > width && has_break) {
      ...
      for (uint32_t j = last_break_idx; j <= i; j++) {
        ...
        col += (cp2 >= 0) ? n00b_unicode_char_width((n00b_codepoint_t)cp2) : 1;
      }
    }
  }
...
uint32_t *n00b_unicode_linebreak_wrap(n00b_string_t s, uint32_t *num_breaks) _kargs {
  int width = 80;
  n00b_allocator_t *allocator = nullptr;
}
{
  (void)allocator;
  return n00b_unicode_linebreak_wrap_raw(s.data, s.u8_bytes, width, num_breaks);
}
