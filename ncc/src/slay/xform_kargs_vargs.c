...
find_synthetic_identifier(n00b_parse_tree_t *node)
{
    ...
    size_t nc = n00b_tree_num_children(node);
    for (size_t i = 0; i < nc; i++) {
        n00b_parse_tree_t *r = find_synthetic_identifier(
...
find_leaf_with_text(n00b_parse_tree_t *node, const char *text)
{
    ...
    size_t nc = n00b_tree_num_children(node);
    for (size_t i = 0; i < nc; i++) {
        n00b_parse_tree_t *r = find_leaf_with_text(n00b_tree_child(node, i),
                                                    text);
        if (r) {
            return r;
        }
...
param_list_has_plus(n00b_parse_tree_t *declarator)
{
    ...
    // and look for a "+" leaf.
    n00b_parse_tree_t *fd = n00b_xform_find_child_nt(declarator,
                                                      "function_declarator");
    if (!fd) {
        // Might be deeper (pointer -> direct_declarator -> function_declarator)
        n00b_parse_tree_t *dd = n00b_xform_find_child_nt(declarator,
                                                          "direct_declarator");
        if (dd) {
...
find_func_declarator(n00b_parse_tree_t *declarator)
{
    ...
    for (size_t i = 0; i < nc; i++) {
        n00b_parse_tree_t *r = find_func_declarator(
            n00b_tree_child(declarator, i));
        if (r) {
            return r;
        }
    }
                             ...
                             int *out_count, int *out_cap)
{
    ...
    if (n00b_xform_nt_name_is(node, "keyword_param")) {
        ...
        n00b_parse_tree_t *ds = n00b_xform_find_child_nt(node,
        ...
        if (*out_count >= *out_cap) {
            ...
            *out_params = realloc(*out_params,
                                  sizeof(ncc_kw_param_t) * (size_t)*out_cap);
        }
    ...
    size_t nc = n00b_tree_num_children(node);
    for (size_t i = 0; i < nc; i++) {
        collect_kw_params_from_list(n00b_tree_child(node, i),
                                    out_params, out_count, out_cap);
    }
}
...
static n00b_parse_tree_t *
generate_kargs_struct(n00b_grammar_t *g, const char *func_name,
                       ncc_kw_info_t *kw)
{
...
rewrite_vargs_params(n00b_grammar_t *g, n00b_parse_tree_t *declarator)
{
    ...
    if (pl_text && strlen(pl_text) > 0) {
        snprintf(new_ptl_src, sizeof(new_ptl_src),
                 "%s , n00b_vargs_t * vargs", pl_text);
    }
    else {
        snprintf(new_ptl_src, sizeof(new_ptl_src),
                 "n00b_vargs_t * vargs");
    ...
    n00b_parse_tree_t *new_ptl = parse_template(g, "parameter_type_list",
                                                 new_ptl_src);
    if (!new_ptl) {
        return;
    }
...
static void
add_kargs_param(n00b_grammar_t *g, n00b_parse_tree_t *declarator,
                const char *func_name)
{
    ...
    if (existing_text && strlen(existing_text) > 0) {
        snprintf(new_ptl_src, sizeof(new_ptl_src),
                 "%s , struct _%s__kargs * kargs", existing_text,
                 func_name);
    }
    else {
        snprintf(new_ptl_src, sizeof(new_ptl_src),
                 "struct _%s__kargs * kargs", func_name);
    }
    free(existing_text);
...
static void
inject_kargs_body(n00b_grammar_t *g, n00b_parse_tree_t *func_body,
                   ncc_kw_info_t *kw)
{
    // Find compound_statement -> block_item_list
    n00b_parse_tree_t *compound = n00b_xform_find_child_nt(func_body,
                                                            "compound_statement");
    if (!compound) {
        return;
    }

    n00b_parse_tree_t *bil = n00b_xform_find_child_nt(compound,
                                                       "block_item_list");
    ...
    for (int i = kw->num_params - 1; i >= 0; i--) {
        ...
        n00b_parse_tree_t *block_item = parse_template(g, "block_item",
                                                        decl_src);
        if (!block_item) {
            continue;
        }
        ...
        if (bil) {
            ...
            bil = n00b_xform_make_node_with_children(
                g, "block_item_list", 0, children, 1);
...
static void
insert_struct_before(n00b_parse_tree_t *ext_decl,
...
find_decl_declarator(n00b_parse_tree_t *node)
{
    ...
    n00b_parse_tree_t *idl = n00b_xform_find_child_nt(node,
...
xform_funcdef(n00b_xform_ctx_t *ctx, n00b_parse_tree_t *node)
{
    ...
    if (nc == 2) {
        ...
        if (c0 && n00b_tree_is_leaf(c0)
            && n00b_xform_leaf_text_eq(c0, "__extension__")) {
            real_fd = n00b_tree_child(real_fd, 1);
            if (!real_fd || !n00b_xform_nt_name_is(real_fd,
                                                    "function_definition")) {
                return NULL;
            }
        }
    ...
    n00b_parse_tree_t *declarator = n00b_xform_find_child_nt(real_fd,
                                                              "declarator");
    if (!declarator) {
        return NULL;
    }
    ...
    if (has_kw) {
        n00b_parse_tree_t *kw_clause = n00b_xform_find_child_nt(real_fd,
                                                    "keyword_clause");
        ...
        if (!is_opaque) {
            ...
            n00b_parse_tree_t *kpl = n00b_xform_find_child_nt(kw_clause,
                                                    "keyword_param_list");
            if (!kpl) {
                uint32_t line, col;
            ...
            if (!meta->kw) {
                ...
                n00b_parse_tree_t *pl  = ptl
                    ? n00b_xform_find_child_nt(ptl, "parameter_list")
                    : NULL;
                ...
                if (param_count > 0) {
                    ...
                    for (int i = 0; i < param_count; i++) {
                        ...
                        for (int j = 0; j < existing->num_params; j++) {
                            if (strcmp(params[i].name,
                                       existing->params[j].name) == 0) {
                                found = true;
                    ...
                    for (int j = 0; j < existing->num_params; j++) {
                        ...
                        if (!found) {
                            fprintf(stderr,
                                ...
                                "missing parameter '%s'\n",
                                func_name, existing->params[j].name);
                        }
                    }
                }
        ...
        if (meta->kw && !meta->kw->is_opaque && !meta->kw->struct_emitted) {
            n00b_parse_tree_t *struct_node = generate_kargs_struct(
                ctx->grammar, func_name, meta->kw);
            if (struct_node) {
    ...
    n00b_parse_tree_t *func_body = n00b_xform_find_child_nt(real_fd,
                                                              "function_body");
        ...
        if (ds) {
            ...
            if (id) {
                ...
                if (meta) {
                    ...
                    n00b_parse_tree_t *call_expr = parse_template(
                        ctx->grammar, "postfix_expression", full_text);
                    free(full_text);

                    if (call_expr) {
                        // Apply call-site transform directly since the
                        // tree walker won't revisit replacement children.
                        n00b_parse_tree_t *xformed = xform_call(ctx,
                                                                  call_expr);
                        if (xformed) {
                            call_expr = xformed;
                        }
                        ...
                        char stmt_src[32768];
                        snprintf(stmt_src, sizeof(stmt_src), "%s ;", ct);
                        free(ct);

                        n00b_parse_tree_t *expr_stmt = parse_template(
                            ctx->grammar, "expression_statement", stmt_src);
                        if (expr_stmt) {
                            ctx->nodes_replaced++;
                            return expr_stmt;
                        }
...
static void
collect_args(n00b_parse_tree_t *node,
             collected_arg_t **out, int *out_count, int *out_cap)
{
    if (!node || n00b_tree_is_leaf(node)) {
        return;
    }
    ...
    if (n00b_xform_nt_name_is(node, "keyword_argument")) {
        ...
        n00b_parse_tree_t *ident_node = n00b_xform_find_child_nt(node,
                                                                   "identifier");
        n00b_parse_tree_t *expr = n00b_xform_find_child_nt(node,
        ...
        const char *kw_name = ident_node ? extract_func_name(ident_node)
                                         : NULL;
...
xform_kw_func(n00b_xform_ctx_t *ctx, n00b_parse_tree_t *arg_list)
{
    ...
    for (int i = 0; i < arg_count; i++) {
        ...
        pos += (size_t)snprintf(literal_src + pos,
                                sizeof(literal_src) - pos,
                                ". _has_%s = 1 , . %s = %s",
                                args[i].name, args[i].name, val_text);
        first = false;
        free(val_text);
    }
    ...
    n00b_parse_tree_t *replacement = parse_template(ctx->grammar,
                                                     "assignment_expression",
                                                     resolved);
    free(resolved);
...
xform_call(n00b_xform_ctx_t *ctx, n00b_parse_tree_t *node)
{
    ...
    if (strcmp(callee, "kw_func") == 0) {
        n00b_parse_tree_t *arg_list = n00b_xform_find_child_nt(
    ...
    n00b_parse_tree_t *arg_list = n00b_xform_find_child_nt(node,
    ...
    if (n_keyword == 0 && meta) {
        ...
        if (!meta->va && !meta->kw) {
            ...
            if (n_positional == expected) {
                ...
                if (!last_pos
                    || !is_kw_func_kargs_literal(last_pos)) {
                    already_done = true;
                }
            }
        }
        ...
        if (kw_func_kargs_text) {
            ...
            for (int i = 0; i < meta->kw->num_params; i++) {
                ...
                if (override_idx >= 0) {
                    ...
                    char *val_text = n00b_xform_node_to_text(
                        args[override_idx].expr);
                    if (!first) {
                        pos += (size_t)snprintf(new_args + pos,
                                                sizeof(new_args) - pos,
                                                " , ");
    ...
    char call_src[32768];
    if (pos > 0) {
        snprintf(call_src, sizeof(call_src), "%s ( %s )", callee_text,
                 new_args);
    }
    else {
...
{
    n00b_xform_register(reg, "function_definition", xform_funcdef,
                          "kargs_vargs_funcdef");
    n00b_xform_register(reg, "declaration", xform_decl,
                          "kargs_vargs_decl");
    n00b_xform_register(reg, "postfix_expression", xform_call,
                          "kargs_vargs_call");
}
