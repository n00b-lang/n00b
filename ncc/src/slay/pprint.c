...
//
// Reads @indent, @group, @concat, @blankline, @softline($N),
// @hardline($N), @nospace($N), @align($N) annotations from the grammar.
...
typedef struct {
    ...
    bool    concat;
    bool    blankline;
    int32_t softline_before[32];
    int32_t hardline_before[32];
    int32_t nospace_before[32];
    int32_t align_to[32];
    int     softline_count;
    int     hardline_count;
    int     nospace_count;
...
read_annotations(n00b_nonterm_t *nt)
{
    ...
    s.hardline_before[0] = -1;
    s.nospace_before[0]  = -1;
    s.align_to[0]        = -1;
...
emit_nt(emit_ctx_t *ctx, n00b_parse_tree_t *node, n00b_nt_node_t *pn)
{
    ...
    if (rule) {
        ...
        // Tier 2: grammar annotations.
        asummary  = read_annotations(nt);
        do_indent = asummary.indent;
        do_group  = asummary.group;
        do_concat = asummary.concat;
        do_blank  = asummary.blankline;
        hardlines = asummary.hardline_before;
        softlines = asummary.softline_before;
        nospaces  = asummary.nospace_before;
        aligns    = asummary.align_to;
    }
