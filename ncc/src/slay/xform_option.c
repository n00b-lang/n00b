...
emit_struct_decl(n00b_xform_ctx_t *ctx, const char *mangled,
                 const char *type_str)
{
    n00b_dict_untyped_t *decls = get_option_decls(ctx);
    if (n00b_dict_untyped_contains(decls, (void *)mangled)) {
        return;
    }
    n00b_dict_untyped_put(decls, strdup(mangled), (void *)(uintptr_t)1);
...
xform_option_type(n00b_xform_ctx_t *ctx, n00b_parse_tree_t *node)
{
    ...
    if (var_name) {
        n00b_dict_untyped_t *meta = get_option_meta(ctx);
        n00b_dict_untyped_put(meta, strdup(var_name),
                   ...
                   bool *out_is_pointer)
{
    ...
    n00b_dict_untyped_t *meta = get_option_meta(ctx);
    bool found;
    void *val = n00b_dict_untyped_get(meta, name, &found);
    if (found) {
