...
{
    sb->cap      = 64;
    sb->buf      = ncc_alloc_array(char, sb->cap);
    sb->len      = 0;
    sb->cp_count = 0;
}
...
sb_push_raw(strbuf_t *sb, char c)
{
    if (sb->len >= sb->cap) {
        ...
        char *new_buf  = ncc_alloc_array(char, new_cap);
...
static void
sb_push_cp(strbuf_t *sb, ncc_codepoint_t cp)
{
    char enc[4];
    uint32_t n = ncc_unicode_utf8_encode(cp, enc);

    for (uint32_t i = 0; i < n; i++) {
        sb_push_raw(sb, enc[i]);
    }
...
{
    ncc_string_t str = ncc_string_from_raw(sb->buf, (int64_t)sb->len);
    ncc_free(sb->buf);
    sb->buf      = nullptr;
    sb->len      = 0;
    sb->cap      = 0;
