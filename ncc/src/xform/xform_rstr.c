...
// =========================================================================
// Must match the struct layout in ncc.c (same pattern as xform_generic_struct.c).
...
static void
parse_rich_markup(const char *desc, int desc_len, rstr_seg_list_t *sl, rstr_text_buf_t *tb)
{
    int i          = 0;
    int text_start = 0;
    ...
    while (i < desc_len) {
        ...
        if (i + 1 < desc_len && (unsigned char)desc[i] == 0xC2
            ...
            while (j + 1 < desc_len
                   && !((unsigned char)desc[j] == 0xC2 && (unsigned char)desc[j + 1] == 0xBB)) {
                j++;
            }
...
pending_push(pending_stack_t *ps, pending_style_t item)
{
    if (ps->count >= ps->cap) {
        ...
        pending_style_t *np      = malloc((size_t)new_cap * sizeof(pending_style_t));
...
static char *
emit_style_declarations(out_style_list_t *out, int uid)
{
    char  *result = NULL;
    size_t size;
...
xform_rstr(ncc_xform_ctx_t *ctx, ncc_parse_tree_t *node)
{
    ...
    char bytes_str[32];
    snprintf(bytes_str, sizeof(bytes_str), "%d", tb.len);
    ...
    fputc('"', df);
    emit_escaped_string(df, tb.data, tb.len);
    fputc('"', df);
    fclose(df);
